...
struct ContentView: View {
    ...
    @State private var editingEntry: GigwageEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Log earnings from odd jobs and side gigs outside your main paycheck.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: GigwageEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (GigwageEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: GigwageEntry? = nil, onSave: @escaping (GigwageEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = GigwageEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
